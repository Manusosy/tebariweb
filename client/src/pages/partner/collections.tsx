...
export default function PartnerCollections() {
  ...
  const readyForCollection = MOCK_HOTSPOTS.filter(h => h.estimatedVolume > 100 || h.severity === 'critical');
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant={spot.severity === 'critical' ? 'destructive' : 'default'} className="uppercase text-[10px]">
                        {spot.severity} Priority
                      </Badge>
                    </div>
                    <div className="flex items-center gap-4 text-sm text-muted-foreground">
                      <span className="flex items-center gap-1">
                        <MapPin className="h-4 w-4" /> Zone {spot.id.toUpperCase()}
                      </span>
                      <span className="flex items-center gap-1">
